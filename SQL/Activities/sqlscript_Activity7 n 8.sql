...
REM   Sql script for Activity 7 and 8
...
select min(purchase_amount) as "Minmum Purchase Amount" from orders;

select count(distinct salesman_id) as "Total number_of salesmen" from orders;

select customer_id, max(purchase_amount) as "Purchase Amount" from orders group by customer_id;

select salesman_id, order_date, max(purchase_amount) as "Purchase Amount" from  orders where order_date=TO_DATE('2012/08/17', 'YYYY/MM/DD') group by salesman_id, order_date;
...
select salesman_id, order_date, max(purchase_amount) as "Purchase Amount" from  orders 
